...
###
 ...
 # @LastEditTime: 2025-09-28 17:39:38
 # @FilePath: /VGSC/scripts/eval_with_1_ply_dir.sh
 # @Description: 
 # 
...
ORI_RENDER_DIR=$(echo "$PLY_DIR" | sed 's|^data|renders/gsplat|')  # replace 'data' with 'renders/gsplat'
RESULT_DIR="$(dirname "$ORI_RENDER_DIR")/$ply_dir_name" # replace 'track' with ply_dir_name
COMPRESSED_PLY_DIR=data/GSC_splats/m71763_${dataset}_stable/${ply_dir}
...
run_experiment() {
    ...
    CUDA_VISIBLE_DEVICES=${gpu_id} python gs_pipeline.py \
        ...
        --compressed_ply_dir $COMPRESSED_PLY_DIR \
