    ...
    for metric in metrics:
        ...
        for json_path, label in zip(rd_json_list, labels):
            ...
            for key, val in sorted(data.items()):
                if to_MB:
                    if "bytes" in val:
                        bitrates.append(val["bytes"] / 1e6)  # Convert bytes to MB
                    elif "bitrate" in val:
...
def main():
    ...
    for target in args.target_files:
        ...
        # sorted_list = ['GSCodecStudio-ZJU (m74704)', 'VPCC-ByteDance (m74012)', 'VPCC-Nokia (m73977)',  'UniGSC-video_ctc', 'UniGSC-video_enhanced', 'VGSC (ours)']
        ...
        plot_rd_curve(matched_files, labels, output_root_dir, target, args.to_MB)
